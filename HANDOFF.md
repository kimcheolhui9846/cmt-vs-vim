...
4. **지표별 수렴(N=512, 마지막 두 점 상대 변화 ≤5%)**:
   ...
   - `principal_angle_deg`: **8/9 미수렴, 1/9(`cmt_s`/`noise`, +1.2%) 수렴.**
     미수렴 8건은 Task 4가 확립한 캐비아트(지수가 1에 가까우면 주축이 수치적으로
     ill-conditioned)와 정확히 부합한다 — 애초에 방향 근거로 쓰지 않는 지표이므로
     대체로 수렴하지 않는다는 사실 자체가 그 캐비아트를 뒷받침한다. 우연히 수렴한
     `cmt_s`/`noise` 한 건도 이 캐비아트를 뒤집는 근거로 쓰지 않는다(`noise`
     조건은 애초에 정직성 게이트·논문 인용 대상이 아니다).
