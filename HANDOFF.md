...
2. **Vim은 크롭 48 이상 전 구간에서 가장 높다** — 전체 맵(224)을 포함해서.
   그리고 Vim의 값은 크롭에 가장 둔하다(1.045–1.138).
...
- **주축이 여섯 개 N 전부에서 수평 0.07°~0.11° 안에 있다.** 전 구간 폭이
  0.039°다. 무작위였다면 [0, 90] 어딘가에 흩어졌을 값이 0에 붙어 있다.
...
실행에서 **같은 입력**으로 잰 셀이다. 그 10개 셀의 실행 간 차이는
`anisotropy` 최대 1.1e-06, `decay_ratio` 최대 8.3e-06으로 자연 이미지 셀보다
네 자릿수 작다(10개 중 5개는 완전히 동일). 나머지 1e-6은 `patch_embed`의 conv
backward가 비결정적이라 남는 것이며, 위 percent 수준 변동과는 무관하다. 요컨대
**입력이 같으면 1e-6, 이미지가 바뀌면 percent** — 변동의 출처는 샘플링이다.
...
> 타원의 주축이 수평에서 0.1° 이내에 정렬된다** — 표본 16장부터 512장까지 여섯
> 개 표본 크기 전부에서 0.07°~0.11° 안에 머문다. 지수가 1에서 충분히 멀어
> 주축 방향이 수치적으로 잘 정의되는 조건이며, 이는 논문이 서술로 주장한
> "scan 방향에 정합된 ERF 타원"을 문자 그대로 직접 측정한 값이다. 같은 조건에서
> 감쇠비도 1.90으로 학습된 모델(1.35)보다 오히려 크다. 즉 이 비등방성의 원인은
> 학습된 표현이 아니라 row-major flatten이라는 **1D scan 구조 자체**에 있다.
