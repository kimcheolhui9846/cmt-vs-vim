...
def test_unconverged_rows_are_kept_and_flagged():
    ...
    assert final_metrics(df).iloc[0]["converged"] is np.False_ or (
        final_metrics(df).iloc[0]["converged"] == False  # noqa: E712
    )
...
- [ ] **Step 2: 사용한 이미지 목록을 남기는 코드를 넣는다**

`run_erf`의 env.json 기록 뒤에 추가:

```python
    paths = sample_image_paths(sorted(ensure_voc().glob("*.jpg")), max(sample_sizes), seed=SEED)
    (out_dir / "images.txt").write_text("\n".join(p.name for p in paths))
```

`tests/test_e2.py`에 대응 테스트를 추가한다:
...
def test_the_image_list_is_recorded(tmp_path, monkeypatch):
    ...
    assert len((tmp_path / "images.txt").read_text().splitlines()) == 4
```
...
- [ ] **Step 4: 정직성 검사 — 결과를 믿기 전에 통과해야 한다**
...
1. `status`가 전부 `ok`.
2. 모든 맵의 피크가 중심 ±16 이내. 벗어나면 프로브가 틀린 것이다.
...
여기서의 게이트는 전부 비교 기준이라 달성 가능하다.

- [ ] **Step 5: 사전 등록 예측과 대조한다**
