...
SAMPLE_SIZES = (16, 32, 64, 128, 256, 512)  # Task 9 리뷰에서 512 추가
SEED = 0
...
def test_unconverged_rows_are_kept_and_flagged():
    ...
    수렴 여부를 함께 싣는다. 수렴은 지표마다 독립적으로 판단되므로
    decay_ratio_converged 하나만 False로 두고 나머지는 그대로 둔다."""
...
3모델 × 3조건 × 6개 N(16~512). 가장 큰 셀이 512장 forward+backward이므로 모델당
몇 분이면 끝난다. E1의 sweep보다 훨씬 가볍다.
...
cols = ['model','condition','anisotropy','anisotropy_converged','anisotropy_central',
        'anisotropy_central_converged','principal_angle_deg','principal_angle_converged',
        'decay_ratio','decay_ratio_converged']
print(final[cols].to_string(index=False))

for metric in ['anisotropy_converged','anisotropy_central_converged',
               'principal_angle_converged','decay_ratio_converged']:
    unconverged = final[~final[metric].fillna(False).astype(bool)][['model','condition']]
    print(metric, '미수렴:', list(unconverged.itertuples(index=False)))
...
(`converged` 단일 열은 더 이상 없다 — 지표별로 `anisotropy_converged`·
`anisotropy_central_converged`·`principal_angle_converged`·`decay_ratio_converged`
네 열이 따로 있다. `status`가 `error`인 행은 지표 열이 전부 비어 있으므로
`fillna(False)`로 "미수렴"에 넣는다 — 애초에 값이 없는 것과 값은 있는데
안정되지 않은 것을 이 출력에서는 구분하지 않는다.)
