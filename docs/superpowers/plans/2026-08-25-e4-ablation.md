...
> **이 문서 이후 바뀐 것.** 이 계획은 구현 당시 기록이며 아래 몸통은 고치지 않는다.
> 레시피의 현재 권위는 설계 문서(`docs/superpowers/specs/2026-08-25-e4-ablation-design.md`)다.
> 이 문서와 실제로 갈린 곳 둘:
> - 1049번째 줄의 "원 DeiT 레시피에서 벗어나는 곳은 crop_scale 하나뿐이다"는
>   더 이상 맞지 않는다. weight decay 제외 집합에서 B·D의 mamba `A_log`·`A_b_log`가
>   빠지는 것이 두 번째 편차로 자리잡았다 (`configs/e4_common.yaml`, 설계 문서의
>   레시피 편차 절, `bench/train.py`의 `param_groups`).
> - `TrainConfig`(1137·1253번째 줄 부근)는 여전히 `drop_path` 필드를 보여 준다.
>   지금 `bench/train.py`의 `TrainConfig`는 그 필드를 갖지 않는다 — `drop_path`는
>   오케스트레이터(`experiments/e4_ablation.py`)가 `common["drop_path"]`를
>   `models.registry.build_e4_model`에 직접 넘기는 경로 하나뿐이다.
