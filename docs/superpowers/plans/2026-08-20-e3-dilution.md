...
- **코드 주석과 docstring은 한글, 그림에 그려지는 문자열과 CSV에 들어가는 범주 값은
  ...
  범주 값이란 `area_bin`·`aspect_class`·`status`처럼 그림의 축이나 범례로 올라가는
  값이다. **진단 메시지는 여기 해당하지 않는다** — `error` 열에는 저장소의 가드가
  내는 한글 예외 메시지를 그대로 담는다. 그림이 그 열을 읽지 않으므로 폰트 문제가
  없고, 이미 병합된 `results/e2/erf_metrics.csv`의 `error` 값 여섯 개가 같은 형식이라
  E3만 영어로 바꾸면 두 실험의 같은 이름 열이 다른 언어가 된다.
...
- Produces:
  ...
  - `expected_cells(model_names: tuple[str, ...], conditions: tuple[str, ...]) -> set[tuple[str, str]]`
  - `common_subset(df: pd.DataFrame, cells: set[tuple[str, str]]) -> pd.DataFrame`
...
교체하고, 이어서 `missing` 검사를 지운 채 `len(cells)` 대신 df에서 유추한 개수를
쓰도록 되돌려 본다.

Run: `pytest tests/test_coverage.py -v`
Expected: 각각 `test_common_subset_keeps_only_instances_every_cell_measured`와
`test_common_subset_fails_when_a_model_produced_no_rows_at_all`이 FAIL.
확인 후 되돌리고 다시 통과시킨다.
...
Expected: `test_vim_sequence_index_shifts_only_after_the_inserted_cls` **하나만** FAIL.

`>=`와 `>`는 `patch_index == 98`에서만 갈리는데, 다른 테스트가 집는 좌표는 39·105·156이라
전부 그 경계를 비켜간다. 그래서 이 경계 단위 테스트가 없으면 **이 회귀를 잡는 테스트가
하나도 없다** — 실측으로 확인했다. 다른 테스트도 함께 실패할 것이라 기대하지 말 것.
확인 후 되돌리고 다시 통과시킨다.
...
**Files:**
- Create: `experiments/e3_dilution.py` (목록 부분만), `tests/vocfixtures.py`
- Modify: `tests/test_voc_masks.py` (인라인 팔레트 코드를 공용 헬퍼 호출로)
- Test: `tests/test_e3.py`

**Interfaces:**
- Consumes: `data.voc_masks`, `bench.coverage`의 `area_bin`/`aspect_ratio`/
  `aspect_class`/`object_pixels`/`population_size`/`query_patch`/`AREA_BINS`,
  `models.probes.PATCH_GRID_AT_224` (Task 5가 만든다)
- Produces:
  - `tests/vocfixtures.py`의 `write_mask_png(path: Path, mask: np.ndarray) -> Path`
  - `MIN_MASK_PIXELS: int = 512`
  - `INSTANCES_PER_BIN: int = 100`
  - `QUERY_GRIDS: tuple[int, ...]` — 세 모델이 쓰는 서로 다른 격자, 즉 `(7, 14)`
  - `SEED: int = 0`
  - `CATALOG_COLUMNS: list[str]`
  - `queryable_column(grid: int) -> str`
  ...
  - `sample_instances(catalog: pd.DataFrame, per_bin: int = INSTANCES_PER_BIN, seed: int = SEED) -> pd.DataFrame`

- [ ] **Step 1: 마스크 PNG 작성기를 공용으로 꺼낸다**

Task 1이 `tests/test_voc_masks.py` 안에 인라인으로 두었던 팔레트 처리를 두 번째
소비자가 생긴 지금 공용 모듈로 옮긴다. 팔레트를 빠뜨리면 조용히 데이터가
손상되므로, 이 코드가 두 곳에 복제되면 한쪽만 고쳐질 자리가 생긴다.

`tests/vocfixtures.py`:
...
`tests/test_voc_masks.py`의 `_synthetic_pair`에서 인라인 팔레트 코드를 지우고
`write_mask_png(tmp_path / "sample_mask.png", mask)` 호출로 바꾼다. 그 파일의
`test_the_synthetic_mask_really_round_trips_its_void`가 이 헬퍼의 회귀 테스트
역할을 그대로 이어받으므로 새 테스트는 필요 없다.

Run: `pytest tests/test_voc_masks.py -v`
Expected: 9 passed (Task 1과 동일 — 리팩터가 동작을 바꾸지 않았다는 뜻이다)
...
def test_instances_no_model_can_query_are_never_sampled():
    ...
    # 한 칸 걸러 표시한다. 앞에서부터 200행을 잡으면 여섯 구간 중 넷이 통째로
    # 비어 sample_instances가 필터를 시험하기도 전에 예외를 던진다 — 그러면 이
    # 테스트는 자기가 막는다고 주장하는 것을 한 번도 실행하지 않는다.
...
- [ ] **Step 6: 실제 VOC 전체로 목록을 만들어 분포를 본다**
...
print('eligible instances', len(cat), '| measurable by all grids', int(cat.measurable_by_all.sum()))
print(cat.groupby('area_bin').measurable_by_all.agg(['size','sum']))
s = sample_instances(cat)
print('sampled', len(s)); print(s.groupby('area_bin').size()); print(s.groupby('aspect_class').size())
"
```

고정 환경에서 미리 재 둔 값이다. 크게 어긋나면 멈추고 원인을 찾을 것:

| 면적 구간 | 자격 | 세 격자 모두 질의 가능 |
|-----------|------|------------------------|
| `<2%`     | 568  | 114 |
| `2-5%`    | 984  | 728 |
| `5-10%`   | 876  | 849 |
...
표본은 600개(구간당 100), 종횡비는 세 범주 모두 비어 있지 않아야 한다.

- [ ] **Step 7: 회귀 주입 — 정렬 테스트가 순회 순서 의존을 잡는지 확인한다**
...
- Produces:
  ...
  - `run_dilution(model_names=MODEL_NAMES, conditions=CONDITIONS, per_bin=INSTANCES_PER_BIN, out_dir="results/e3") -> pd.DataFrame`
...
Run: `pytest tests/test_e3.py -v`
Expected: 14 passed (Task 6의 9개 + 이 태스크의 5개)
...
df = run_dilution(per_bin=2, out_dir='/tmp/e3-smoke')
...
- Produces:
  ...
  - `bin_positions(labels) -> list[int]` — 각 라벨의 `AREA_ORDER` 상 위치
  - `baseline_series(summary: pd.DataFrame) -> pd.DataFrame` — 구간별 기준선 한 계열
...
그 다음, 따로 `_draw_bins`에서 `bin_positions(...)` 대신 `series["area_bin"]`을
그대로 넘기고 `set_xticks`/`set_xticklabels` 두 줄을 지워 본다. **이 회귀는
테스트가 잡지 못한다** — `bin_positions`는 순수 함수라 그대로 통과한다. 그러니
눈으로 확인한다: `python -m figures.e3_plot`으로 그림을 만들고 x축 눈금이 여섯
구간 전부를 면적 순서로 보여주는지 본다. 확인 후 되돌린다.
확인 후 되돌리고 다시 통과시킨다.
...
공통 부분집합 크기와 `no_query_patch` 건수가 출력된다. 3모델 × 2조건 × 600
인스턴스(구간당 100 × 6구간) = 3600행이다.

표본을 뽑을 때 이미 세 격자 모두 질의 가능한 것만 골랐으므로 `no_query_patch`는
0에 가까워야 한다. 많이 나오면 `build_catalog`의 판정과 Task 7의 판정이 어긋난
것이므로 멈추고 원인을 찾을 것 — 두 곳이 같은 `query_patch`를 같은 마스크에
적용하는데 결과가 다르다면 마스크가 두 경로에서 다르게 만들어지고 있다는 뜻이다.
...
python -c "
import pandas as pd
from bench.coverage import common_subset, aggregate, expected_cells
from experiments.e3_dilution import CONDITIONS
from models.registry import MODEL_NAMES
...
1. `precision_mean`이 어느 셀에서든 `baseline_mean`에 붙어 있으면 그 셀은 신호가
   없는 것이고, 그렇게 보고한다.
2. `no_query_patch`는 0에 가까워야 한다 — 표본이 이미 세 격자 모두 질의 가능한
   인스턴스로 층화되어 있기 때문이다. 0이 아니면 목록 단계와 측정 단계의 판정이
   어긋난 것이다.
3. `low_sample`은 어느 구간에서도 True가 아니어야 한다(구간당 100개). True가
   있으면 그 구간에서 측정이 실패한 것이므로 `status` 열을 먼저 본다.
...
from bench.coverage import expected_cells
from experiments.e3_dilution import CONDITIONS
from models.registry import MODEL_NAMES
df = common_subset(pd.read_csv('results/e3/coverage.csv'), expected_cells(MODEL_NAMES, CONDITIONS))
