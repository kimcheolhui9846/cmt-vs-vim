...
- Consumes: 없음 (numpy만)
- Produces:
  - `PATCH_GRID: dict[str, int]` — `{"deit_s": 14, "vim_s": 14, "cmt_s": 7}`
...
def test_query_patch_stays_inside_the_mask():
    ...
    mask = _u_shaped_mask()
    for grid in sorted(set(PATCH_GRID.values())):
...
"""224²에서 각 모델의 최종 격자 한 변.

DeiT와 Vim은 patch16이라 14×14, CMT는 4단계 다운샘플 끝이라 7×7이다. 격자가
다르면 같은 객체라도 질의 후보의 개수가 달라진다 — CMT의 셀은 32×32라 작은
객체가 어떤 셀도 과반으로 덮지 못한다. 이 비대칭은 숨기지 않고 Task 7에서
모델별 제외 수로 기록하고, 집계는 세 모델 전부에서 질의를 찾은 공통 부분집합만
쓴다(`common_subset`).

이 상수는 추측이 아니라 `models/probes.py`가 실제로 캡처하는 특징맵에서 나온
값이고, `tests/test_probes.py`가 모델을 돌려 그 일치를 단언한다.
"""
...
def test_the_declared_patch_grid_matches_what_the_model_produces():
    """bench/coverage.py의 PATCH_GRID는 이 격자를 전제로 질의 후보를 고른다.
    어긋나면 마스크 안이라고 고른 좌표가 다른 토큰을 가리킨다."""
    from bench.coverage import PATCH_GRID
    ...
    for name in MODEL_NAMES:
        ...
        measured = feature_grid(name, model, x)
        assert measured == PATCH_GRID_AT_224[name] == PATCH_GRID[name], name
...
"""224² 입력에서 각 모델이 내놓는 최종 격자 한 변.

`bench/coverage.py`의 `PATCH_GRID`와 같은 값이어야 하고, 두 상수가 어긋나면
마스크 안이라고 고른 좌표가 다른 토큰을 가리킨다.
`tests/test_probes.py::test_the_declared_patch_grid_matches_what_the_model_produces`가
실제 모델을 돌려 셋의 일치를 단언한다 — 이 값은 추측이 아니라 실측이다.
"""
...
def test_measure_instance_query_patch_is_inside_the_mask(tmp_path, monkeypatch):
    ...
    import experiments.e3_dilution as e3
    from bench.coverage import MIN_PATCH_COVERAGE, PATCH_GRID, patch_coverage
    ...
    coverage = patch_coverage(mask == 1, PATCH_GRID["vim_s"])
...
def measure_instance(
    ...
    try:
        ...
        patch = query_patch(obj, PATCH_GRID[model_name])
        if patch is None:
...
- `PATCH_GRID`(`bench/coverage.py`)와 `PATCH_GRID_AT_224`(`models/probes.py`)는
  같은 값을 갖는 별개 상수다. 두 계층이 서로를 import하지 않게 하려고 나눴고,
  일치는 `tests/test_probes.py::test_the_declared_patch_grid_matches_what_the_model_produces`가
  실제 모델을 돌려 단언한다.
