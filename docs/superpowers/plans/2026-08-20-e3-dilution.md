...
Expected: `test_vim_sequence_index_shifts_only_after_the_inserted_cls`가 FAIL
(경계 98을 직접 못박으므로 이 회귀를 정확히 잡는다). `test_query_tokens_look_at_their_own_patch`와
`test_vim_center_index_skips_the_inserted_cls_token`도 함께 FAIL한다.
확인 후 되돌리고 다시 통과시킨다.
