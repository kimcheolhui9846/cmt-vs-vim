# Vendored from hustvl/Vim:
# https://github.com/hustvl/Vim/blob/main/vim/models_mamba.py
# Commit: b9cf48f1d6d1693308407b304222cfdcd863297b  (2024-08-02)
# Retrieved: 2026-08-18
#
# **상류 main 이 아니라 이 커밋으로 고정한 이유.** main 에는 56a28e0
# "add_rocm_support"(2024-09-10)가 들어와 있고, 이 커밋이 triton layer_norm 과
# selective_scan CUDA 소스를 함께 갈아엎었다. 그 결과 CUDA 경로에서 fused add-norm
# 커널이 컴파일되지 않는다 — BLOCK_N 을 autotune/heuristics 로 감싼 커널에 위치
# 인자로 넘겨 constexpr 슬롯이 어긋난다(triton 2.1.0 · 2.2.0 양쪽 동일 실패).
# fused_add_norm 을 끄는 우회는 latency 측정을 오염시키므로 쓸 수 없다.
# b9cf48f 는 ROCm 직전 커밋이라 논문 시기의 CUDA 경로를 그대로 유지한다.
# mamba-1p1p1 도 반드시 같은 커밋에서 빌드해야 한다 — 이 파일이 import 하는
# mamba_ssm.ops.triton.layernorm 은 ROCm 커밋에서 layer_norm 으로 이름이 바뀐다.
#
# 원본과 한 줄만 다르다: 원본의 `from rope import *` 는 vim/ 디렉터리를 sys.path 에
# 올려두고 실행하는 것을 전제한다. 이 저장소에서는 패키지 경로가 달라
# `from models.rope import *` 로 바꿨다. rope.py 도 models/rope.py 로 함께 벤더링했다.
#
# timm 0.x import 경로(timm.models.layers / timm.models.registry)는 그대로 둔다 —
# 고정 환경의 timm 0.9.12 에서 이것이 정식 경로다. models/cmt_official.py 와 같은 원칙.
#
# 이 파일의 Vim-S 팩토리는
#   vim_small_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2
# 이며 논문 표 2의 Vim-S(26M, ImageNet 80.5%)에 해당한다.
