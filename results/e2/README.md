...
같은 고정 환경 안에서도 backward가 비트 단위로 결정적이지는 않다.
`patch_embed`의 conv backward가 비결정적이라, 완전히 같은 입력으로 두 번 재면
지표가 1e-6 수준에서 달라진다(두 실행 대조는 `HANDOFF.md`의 "두 실행 대조가
말해 주는 것" 절 참고). 이 크기는 결론에 영향을 주지 않지만 "비트 단위 재현"을
주장하지는 말 것.
