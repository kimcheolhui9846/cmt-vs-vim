...
**이 관문이 실제로 잡는 것**: 발산하는 학습 루프(200장도 못 외움), 얼어붙었거나
...
**이 관문이 잡지 못하는 것**: (1) 본 학습 레시피의 lr이 틀린 것 — 이 함수는
`configs/e4_common.yaml`을 읽지 않고 자기 lr(1e-3)을 쓴다. yaml의 lr이 잘못 박혀
있어도 이 관문은 그것을 보지 못한 채 통과한다. 여기서 레시피 lr을 쓰도록 올리지
않는 이유는, 그 값이 200장 암기용으로 고른 값이 아니어서 관문이 재려는 것("배선이
살아 있는가")이 아니라 "레시피 lr로 200장을 외울 수 있는가"를 재게 되기 때문이다.
레시피 lr은 `tests/test_e4_widths.py`의 설정 고정 테스트가 지킨다. (2) 검증(val)
분할의 라벨 매핑 어긋남 — 이 함수는 `train` 분할에서 `ImageFolder`로 이미지와
라벨을 같은 호출로 얻으므로, 그 매핑이 의미상 틀려도 내부적으로는 일관돼 200장을
그대로 외운다. val 라벨 매핑은 `tests/test_tiny_imagenet.py`의 몫이다.
(3) augmentation이 라벨과 어긋나는 경우 — augmentation과 mixup을 의도적으로 꺼
두므로(아래) 그 경로 자체가 이 관문에서 전혀 실행되지 않는다.
